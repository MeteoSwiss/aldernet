...
from ray import init, shutdown, train, tune
...
# First-party
from aldernet.data.data_utils import Batcher, Stations
...
def load_data(settings, debug_mode=False, debug_data_size=100):
    ...
    Args:
        ...
        debug_mode (bool): Whether to load a reduced dataset for debugging.
        debug_data_size (int): Number of samples to load in debug mode.

    Returns:
        Tuple[xr.Dataset, xr.Dataset]: A tuple containing the training and validation data.
    """
...
def train_and_evaluate_model(run_path, settings, sha, debug_mode=False):
    ...
    Args:
        ...
        settings (dict): Dictionary of settings.
        sha (str): A string containing the Git commit hash for the current code.
        debug_mode (bool): Whether to run in debug mode with reduced dataset and more logs.
...
def train_model(config, generator, data_train, data_valid,
                run_path, noise_dim, add_weather, shuffle):
    # Setup for training
    ...
    while True:
        ...
        loss_valid, step_valid = validate_epoch(
            generator, data_valid, noise_dim, add_weather, center, scale, epoch,
            step_valid, run_path, tune_trial)
