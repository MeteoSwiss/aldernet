---
title: "Verification of the Machine Learning Model for Surface Level Alder Pollen"
author: "Simon Adamov"
...
The notebook is an adjusted version of the one used for this paper (XXX Todo), and might still
...
- Kenda hourly analysis (COSMO-1E) for Alnus
- Kenda hourly analysis (COSMO-1E) for Corylus: Simply assuming that both species follow the
...
verification method has been widely accepted for pollen concentrations.
...
col_names <- c("measurement", "cosmo_alnu", "cosmo_cory", "ml")
col_hex <- c("#3b3a3a", "#f16863", "#f7e336", "#58c9dd")
...
cols <- c("taxon", cols)
datapath <- paste0(here(), "/data/22_alnu_dwh.atab")
data_dwh <- read_table(paste(datapath), skip = 17) %>%
  ...
  select(all_of(cols)) %>%
  filter(taxon == "Alnus" & between(
    datetime,
    ymd_hms("2022-02-09 00:00:00"),
    ymd_hms("2022-03-31 23:00:00")
  )) %>%
...
datapath <- paste0(here(), "/data/22_alnu_cosmo.atab")
data_cosmo_alnu <- read_table(paste(datapath), col_names = TRUE, skip = 18) %>%
  pivot_longer(PBS:PZH,
    names_to = "station_tag",
    values_to = "value"
  ) %>%
  mutate(type = "cosmo_alnu") %>%
  inner_join(species, by = c("PARAMETER" = "cosmo_taxon")) %>%
...
datapath <- paste0(here(), "/data/22_cory_cosmo.atab")
data_cosmo_cory <- read_table(paste(datapath), col_names = TRUE, skip = 18) %>%
  pivot_longer(PBS:PZH,
    names_to = "station_tag",
    values_to = "value"
  ) %>%
  mutate(type = "cosmo_cory") %>%
  inner_join(species, by = c("PARAMETER" = "cosmo_taxon")) %>%
  inner_join(stations, by = c("station_tag" = "hirst_station")) %>%
  mutate(
    datetime = ymd_h(paste0(
...
datapath <- paste0(here(), "/data/22_alnu_ml.atab")
data_ml <- read_csv(paste(datapath), col_names = TRUE) %>%
  pivot_longer(PBS:PZH,
    names_to = "station_tag",
    values_to = "value"
  ) %>%
  mutate(type = "ml", taxon = "Alnus") %>%
  inner_join(stations, by = c("station_tag" = "hirst_station")) %>%
  select(taxon, station, datetime, value, type) %>%
  filter(between(
    datetime,
...
data_combined <- data_dwh %>%
  ...
  mutate(type = factor(type,
    ordered = TRUE,
    levels = c("measurement", "cosmo_alnu", "cosmo_cory", "ml")
  ))
...
data_altman <- data_above10 %>%
  pivot_wider(names_from = type) %>%
  select(datetime, measurement, cosmo_alnu, cosmo_cory, ml) %>%
  mutate(
    mean_cosmo_alnu = (measurement + cosmo_alnu) / 2,
    mean_cosmo_cory = (measurement + cosmo_cory) / 2,
    mean_ml = (measurement + ml) / 2,
    diff_cosmo_alnu = cosmo_alnu - measurement,
    diff_cosmo_cory = cosmo_cory - measurement,
...
data_impact_categories <- data_combined %>%
  ...
  mutate(
    ...
    categories_cosmo_alnu = case_when(
      cosmo_alnu < 1 ~ "nothing",
      cosmo_alnu >= 1 & cosmo_alnu < 10 ~ "weak",
      cosmo_alnu >= 10 & cosmo_alnu < 70 ~ "medium",
      cosmo_alnu >= 70 & cosmo_alnu < 250 ~ "strong",
      cosmo_alnu >= 250 ~ "verystrong",
    ),
    categories_cosmo_cory = case_when(
      cosmo_cory < 1 ~ "nothing",
      cosmo_cory >= 1 & cosmo_cory < 10 ~ "weak",
      cosmo_cory >= 10 & cosmo_cory < 70 ~ "medium",
      cosmo_cory >= 70 & cosmo_cory < 250 ~ "strong",
      cosmo_cory >= 250 ~ "verystrong",
    ),
  ...
  mutate_at(
    vars(categories_measurement, categories_cosmo_alnu, categories_cosmo_cory, categories_ml),
...
Each plot shows one species in one year. The separate lines represent individual measurement stations.
...
In this chunk the reader can verify on a per station basis:
...
(gg_boxplot <- data_above10 %>%
  ...
  ggtitle(paste0("Boxplot of Daily ", "*", species_sel, "*", " Pollen Concentrations")) +
...
ci <- map(corr_matrix, ~ .x %>%
  ...
  mutate(
    ...
    comparison = rep(c("mb", "mc", "bc"), times = 6),
...
```{r include=FALSE}
gg_corr1 <- data_corr %>%
  ggplot(aes(x = measurement, y = cosmo_alnu)) +
  geom_point(alpha = 0.3, col = "#222225") +
  xlim(1, 3) +
  ylim(1, 3.5) +
  ...
  ylab("log10(cosmo_alnu)")
```

```{r include=FALSE}
gg_corr2 <- data_corr %>%
  ggplot(aes(x = measurement, y = cosmo_cory)) +
  geom_point(alpha = 0.3, col = "#222225") +
  xlim(1, 3) +
  ylim(1, 3.5) +
  geom_smooth(data = data_corr, alpha = 0.2, col = "#3081b8", fill = "#74cbee") +
...
(gg_corr <- ggarrange(gg_corr1, gg_corr2, gg_corr3, ncol = 3) %>%
  annotate_figure(
    top = text_grob("Pairwise Correlation Between Model and Measurements for ",
...
gg_ab1 <- data_altman %>%
  ggplot(aes(x = mean_cosmo_alnu, y = diff_cosmo_alnu)) +
...
```{r include=FALSE}
gg_ab2 <- data_altman %>%
  ggplot(aes(x = mean_cosmo_cory, y = diff_cosmo_cory)) +
  geom_point(alpha = 0.2, col = "#222225") +
  xlim(0, 500) +
  ylim(-sd_diff[2] * 3, sd_diff[2] * 3) +
  geom_abline(
...
for (j in categs) {
  ...
  gg_conc_box[[j]] <- data_impact_categories %>%
    filter(categories_measurement == j) %>%
    pivot_longer(measurement:ml, names_to = "type", values_to = "Concentration") %>%
    mutate(type = factor(type, levels = c("measurement", "cosmo_alnu", "cosmo_cory", "ml"))) %>%
    ggplot() +
    # The area under that whole curve should be 1.
    # To get an estimate of the probability of certain values,
    # you'd have to integrate over an interval on your 'y' axis,
...
data_impact_categories %>%
  ...
  pivot_longer(diff_cosmo_alnu:diff_ml) %>%
  ...
  ggtitle(paste(
    ...
    species_sel, " Pollen Concentrations"
  )) +
  xlab("") +
...
```{r echo=FALSE}
metrics_cosmo_alnu <- data_above10 %>%
  pivot_wider(names_from = type) %>%
  filter(!is.na(cosmo_alnu)) %>%
  mutate(
    error = cosmo_alnu - measurement,
  ) %>%
  summarise(
    R2 = cor(cosmo_alnu, measurement, method = "spearman", use = "complete.obs")^2,
    Bias = mean(error),
    SD = sd(error),
    MAE = mean(abs(error)),
    RMSE = sqrt(mean((error)^2)),
    MSLE = mean((log10(1 + cosmo_alnu) - log10(1 + measurement))^2, na.rm = TRUE),
    RMSLE = sqrt(MSLE)
  ) %>%
  mutate(type = "cosmo_alnu")

metrics_cosmo_cory <- data_above10 %>%
  pivot_wider(names_from = type) %>%
  filter(!is.na(cosmo_cory)) %>%
  mutate(
    error = cosmo_cory - measurement,
  ) %>%
  summarise(
    R2 = cor(cosmo_cory, measurement, method = "spearman", use = "complete.obs")^2,
    Bias = mean(error),
    SD = sd(error),
    MAE = mean(abs(error)),
    RMSE = sqrt(mean((error)^2)),
    MSLE = mean((log10(1 + cosmo_cory) - log10(1 + measurement))^2, na.rm = TRUE),
    RMSLE = sqrt(MSLE)
  ) %>%
  mutate(type = "cosmo_cory")
...
my_header <- c(species_sel = 8)
names(my_header) <- species_sel

metrics_cosmo_alnu %>%
  bind_rows(metrics_cosmo_cory) %>%
...
```{r echo=FALSE}
matrix_cosmo_alnu <- confusionMatrix(
  data_impact_categories_above10$categories_cosmo_alnu,
  data_impact_categories_above10$categories_measurement
)
kappa_cosmo_alnu <- matrix_cosmo_alnu$overall[1:2] %>%
  tibble() %>%
  mutate(
    type = "cosmo_alnu",
...
matrix_cosmo_cory <- confusionMatrix(
  data_impact_categories_above10$categories_cosmo_cory,
  data_impact_categories_above10$categories_measurement
)
kappa_cosmo_cory <- matrix_cosmo_cory$overall[1:2] %>%
  tibble() %>%
  mutate(
    type = "cosmo_cory",
    metric = c("Accuracy", "Kappa")
...
mae_category <- data_impact_categories_above10 %>%
  select(
    ...
    categories_measurement, categories_cosmo_alnu, categories_cosmo_cory, categories_ml
  ) %>%
  mutate(across(categories_measurement:categories_ml, ~ as.numeric(.x)),
    error_cosmo_alnu = categories_cosmo_alnu - categories_measurement,
    error_cosmo_cory = categories_cosmo_cory - categories_measurement,
    error_ml = categories_ml - categories_measurement
  ) %>%
  summarize(
    MAE_cosmo_alnu = mean(abs(error_cosmo_alnu)),
    MAE_cosmo_cory = mean(abs(error_cosmo_cory)),
...
matrix_cosmo_alnu$byClass %>%
  as_tibble() %>%
  mutate(
    class = factor(rownames(matrix_cosmo_alnu$byClass),
      ordered = TRUE,
      levels = c(
        "Class: nothing",
        "Class: weak",
  ...
  bind_rows(
    matrix_cosmo_cory$byClass %>%
      as_tibble() %>%
      mutate(
        class = factor(rownames(matrix_cosmo_cory$byClass),
          ordered = TRUE,
          levels = c(
            "Class: nothing",
            "Class: weak",
...
```{r echo=FALSE}
decision_num <- metrics_cosmo_alnu %>%
  bind_rows(metrics_ml) %>%
  select(Bias, SD, MAE, RMSLE, type)

change_num <- abs(metrics_ml %>% select(-type)) / abs(metrics_cosmo_alnu %>% select(-type)) - 1
change_num %<>%
...
kappa_cosmo_alnu %>%
  ...
  pivot_wider(names_from = "type") %>%
  mutate(change = abs(ml / cosmo_alnu) - 1) %>%
  bind_rows(
    mae_category %>%
      mutate(metric = "MAE_cat", change = abs(MAE_ml / MAE_cosmo_alnu) - 1) %>%
      select(metric, cosmo_alnu = MAE_cosmo_alnu, ml = MAE_ml, change)
  ) %>%
  ...
  mutate(
    ...
    Kappa = case_when(
      Kappa > -0.2 & type == "change" ~ cell_spec(Kappa, background = "green", color = "white"),
      Kappa <= -0.2 & type == "change" ~ cell_spec(Kappa, background = "red", color = "white"),
      TRUE ~ cell_spec(Kappa)
    ),
    MAE_cat = case_when(
      MAE_cat < 0.2 & type == "change" ~ cell_spec(MAE_cat, background = "green", color = "white"),
      MAE_cat >= 0.2 & type == "change" ~ cell_spec(MAE_cat, background = "red", color = "white"),
      TRUE ~ cell_spec(MAE_cat)
    ),
    Bias = case_when(
      Bias < 0.5 & type == "change" ~ cell_spec(Bias, background = "green", color = "white"),
      Bias >= 0.5 & type == "change" ~ cell_spec(Bias, background = "red", color = "white"),
      TRUE ~ cell_spec(Bias)
    ),
    SD = case_when(
      SD < 0.2 & type == "change" ~ cell_spec(SD, background = "green", color = "white"),
      SD >= 0.2 & type == "change" ~ cell_spec(SD, background = "red", color = "white"),
      TRUE ~ cell_spec(SD)
    ),
    MAE = case_when(
      MAE < 0.2 & type == "change" ~ cell_spec(MAE, background = "green", color = "white"),
      MAE >= 0.2 & type == "change" ~ cell_spec(MAE, background = "red", color = "white"),
      TRUE ~ cell_spec(MAE)
    ),
    RMSLE = case_when(
      RMSLE < 0.1 & type == "change" ~ cell_spec(RMSLE, background = "green", color = "white"),
      RMSLE >= 0.1 & type == "change" ~ cell_spec(RMSLE, background = "red", color = "white"),
